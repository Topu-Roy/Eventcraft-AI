...
function getCategoryBadge(category: string) {
  ...
  return <Badge variant="outline" className={`border ${colorClass} font-medium`}>{label}</Badge>
}
    ...
    return (
      <Link href={`/events/${event._id}`} className="group relative flex gap-4 overflow-hidden border bg-card p-4 transition-all hover:border-primary/50 hover:shadow-sm">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between">
          <div>
            <div className="flex items-start justify-between gap-2"><h3 className="truncate text-sm font-semibold">{event.title}</h3>{getCategoryBadge(event.category)}</div>
            <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{event.description}</p>
          </div>
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span className="flex items-center gap-1"><CalendarDays className="size-3" />{formatEventDate(event.startDatetime)}</span>
            <span className="flex items-center gap-1"><MapPin className="size-3" />{event.venue.city}, {event.venue.country}</span>
  ...
  return (
    <Link href={`/events/${event._id}`} className="group relative flex w-full shrink-0 flex-col overflow-hidden border bg-card transition-all hover:border-primary/50 hover:shadow-sm sm:w-64">
      <div className="relative aspect-video overflow-hidden bg-muted">
        ...
        {isRegistered && <Badge className="absolute top-2 right-2" variant="default"><Ticket className="mr-1 size-3" />Registered</Badge>}
      </div>
      <div className="space-y-2 p-4">
        <h3 className="line-clamp-2 text-sm leading-tight font-semibold">{event.title}</h3>
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><CalendarDays className="size-3.5 shrink-0" /><span>{formatEventDate(event.startDatetime)} · {formatEventTime(event.startDatetime)}</span></div>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><MapPin className="size-3.5 shrink-0" /><span className="truncate">{event.venue.name}, {event.venue.city}</span></div>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><Users className="size-3.5 shrink-0" /><span>{formatCapacity(event.capacity, event.registrationCount)}</span></div>
        </div>
        {event.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-1">
            {event.tags.slice(0, 3).map(tag => <span key={tag} className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">{tag}</span>)}
            {event.tags.length > 3 && <span className="text-[10px] text-muted-foreground">+{event.tags.length - 3}</span>}
          </div>
        )}
      </div>
    </Link>
