...
export default function TicketDetailPage({ params }: { params: Promise<{ ticketCode: string }> }) {
  ...
  const result = useQuery(api.registrations.getByTicketCode, { ticketCode })
  ...
  const response = result as RegistrationResponse | null
  if (!response || response.error || !response.data) {
    return <NotFoundState />
  }

  const { registration, event } = response.data
